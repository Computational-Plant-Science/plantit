...
async def bind(request):
    ...
    if project is not None and study is not None:
        ...
        if study.dataset_paths:
            if not path in study.dataset_paths:
                study.dataset_paths.append(path)
                logger.info(f"Bound {path} to project {project}, study {study}")
            else:
                logger.info(f"Path {path} already bound to project {project}, study {study}")
        else:
...
async def unbind(request):
    ...
    if project is not None and study is not None:
        ...
        if study.dataset_paths:
            if path in study.dataset_paths:
                study.dataset_paths = [p for p in study.dataset_paths if p != path]
                await sync_to_async(study.save)()
                logger.info(f"Unbound {path} from project {investigation.title}, study {study.title}")
            else:
                logger.info(f"Path {path} not bound to project {investigation.title}, study {study.title}")
        else:
            logger.info(f"Path {path} not bound to project {investigation.title}, study {study.title}")
