...
class WorkflowConsumer(WebsocketConsumer):
    ...
    def connect(self):
        ...
        self.logger.info(f"Socket connected for {self.owner}'s workflows")
        async_to_sync(self.channel_layer.group_add)(f"workflows-{self.owner}", self.channel_name)
        self.accept()

    def disconnect(self, code):
        self.logger.info(f"Socket disconnected for {self.owner}'s workflows")
    ...
    def update_workflow(self, event):
        ...
        self.logger.info(f"Pushing {self.owner}'s workflow {workflow['repo']['name']} add/update to client")
    ...
    def remove_workflow(self, event):
        workflow = event['workflow']
        self.logger.info(f"Pushing {self.owner}'s workflow {workflow['repo']['name']} removal to client")
        self.send(text_data=json.dumps({
            'operation': 'remove',
            'workflow': workflow
        }))
