...
def map_old_workflow_config_to_new(old: dict, task: Task, resources: dict) -> dict:
    new_config = {
        'image': old['image'],
    ...
    if 'mount' in old:
        new_config['bind_mounts'] = old['mount']

    if 'parameters' in old:
    ...
    if 'input' in old:
        input_kind = old['input']['kind'] if 'kind' in old['input'] else None
        new_config['input'] = dict()
        if input_kind == 'directory':
            new_config['input']['directory'] = dict()
            new_config['input']['directory']['path'] = join(task.agent.workdir, task.workdir, 'input')
            new_config['input']['directory']['patterns'] = old['input']['patterns']
        elif input_kind == 'files':
            new_config['input']['files'] = dict()
            new_config['input']['files']['path'] = join(task.agent.workdir, task.workdir, 'input')
            new_config['input']['files']['patterns'] = old['input']['patterns']
        elif input_kind == 'file':
            new_config['input']['file'] = dict()
            new_config['input']['file']['path'] = join(task.agent.workdir, task.workdir, 'input',
                                                       old['input']['from'].rpartition('/')[2])
    ...
    if not sandbox and not task.agent.job_array:
        ...
        if task.agent.queue is not None and task.agent.queue != '':
            new_config['jobqueue']['slurm']['queue'] = task.agent.queue
        if task.agent.project is not None and task.agent.project != '':
            new_config['jobqueue']['slurm']['project'] = task.agent.project
        if task.agent.header_skip is not None and task.agent.header_skip != '':
            new_config['jobqueue']['slurm']['header_skip'] = task.agent.header_skip.split(',')

        if 'gpu' in old and old['gpu']:
            if task.agent.gpu:
                print(f"Using GPU on {task.agent.name} queue '{task.agent.gpu_queue}'")
                new_config['gpu'] = True
                new_config['jobqueue']['slurm']['job_extra'] = [f"--gres=gpu:1"]
                new_config['jobqueue']['slurm']['queue'] = task.agent.gpu_queue
            else:
