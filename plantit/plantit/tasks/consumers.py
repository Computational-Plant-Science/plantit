...
class TaskLogConsumer(WebsocketConsumer):
    ...
    def connect(self):
        ...
        self.task_guid = self.scope['url_route']['kwargs']['task_guid']
        self.logger.info(f"Socket connected for user {self.username} task {self.task_guid}")
        async_to_sync(self.channel_layer.group_add)(f"tasks-{self.username}-{self.task_guid}", self.channel_name)
        self.accept()

    def disconnect(self, code):
        self.logger.info(f"Socket disconnected for user {self.username} task {self.task_guid}")
        async_to_sync(self.channel_layer.group_discard)(f"tasks-{self.username}={self.task_guid}", self.channel_name)
