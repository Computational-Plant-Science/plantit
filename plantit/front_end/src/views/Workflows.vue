            ...
            <template v-slot:header style="background-color: white">
                <b-row align-v="center">
                    <b-col class="mt-2" style="color: white">
                        <h5>
                            <i class="fas fa-stream text-dark"></i> Public
                            Workflows
                        </h5>
                    </b-col>
        ...
        <b-card header-bg-variant="light" border-variant="default" class="mt-3">
            <template v-slot:header style="background-color: white">
                <b-row align-v="center">
                    <b-col class="mt-2" style="color: white">
                        <h5>
                            <i class="fas fa-stream text-dark"></i> Your
                            Workflows
                        </h5>
                    </b-col>
                    <b-col md="auto" class="b-form-col">
                        <b-input-group>
                            <b-form-input
                                v-model="filter_user_workflows_query"
                                placeholder="Filter..."
                                class="b-form-input"
                            ></b-form-input>
                            <b-input-group-append>
                                <b-button
                                    :disabled="!filter_user_workflows_query"
                                    @click="filter_user_workflows_query = ''"
                                    variant="white"
                                    >Clear
                                </b-button>
                            </b-input-group-append>
            ...
            <b-row align-h="center">
                <div v-if="!filtered_user_workflows.length">
                    None to show.
                </div>
                <b-card-group deck class="pl-3 pr-3">
                    <b-card
                        v-for="workflow in filtered_user_workflows"
                        :key="workflow.name"
                        class="overflow-hidden p-0 m-2"
                        style="min-width: 30rem"
                    >
...
export default {
    ...
    mounted: function() {
        WorkflowAPI.getWorkflows().then(workflows => {
            this.cps_workflows = workflows || [];
        });
        UserApi.getCurrentUserGithubRepos().then(workflows => {
            this.user_workflows = workflows || [];
        });
    },
    computed: {
        ...
        filter_user_workflows_text: function() {
            return this.filter_user_workflows_query.toLowerCase();
        },
        ...
        filtered_user_workflows: function() {
            if (this.filter_user_workflows_text === '') {
                return this.user_workflows;
            } else {
                return this.user_workflows.filter(w => {
                    return (
                        w.name
                            .toLowerCase()
                            .includes(this.filter_user_workflows_text) ||
                        w.description
                            .toLowerCase()
                            .includes(this.filter_user_workflows_text)
                    );
