...
from plantit_web.misc.models import NewsUpdate, MaintenanceWindow


def updates(request):
    return JsonResponse({'updates': [ModelViews.update_to_dict(u) for u in list(NewsUpdate.objects.all().order_by('created'))]})
