...
![Commit](https://github.com/Computational-Plant-Science/plantit/workflows/RC/badge.svg)
...
Note that `CYVERSE_CLIENT_ID`, `CYVERSE_CLIENT_SECRET`, `CVVERSE_USERNAME`, `CYVERSE_PASSWORD`, `GITHUB_CLIENT_ID`, and `GITHUB_SECRET` must be supplied manually, while `DJANGO_SECRET_KEY`, `DJANGO_FIELD_ENCRYPTION_KEY`, and ``DJANGO_ADMIN_PASSWORD`` will be auto-generated by `scripts/bootstrap.sh` in a clean (empty) install directory.
